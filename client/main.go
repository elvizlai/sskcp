...
func main() {
	...
	flag.BoolVar(&kcpOff, "kcpoff", false, "turn off kcp")

	flag.IntVar(&kcptun.SndWnd, "snd", 128, "set send window size(num of packets)")
	flag.IntVar(&kcptun.RcvWnd, "rcv", 512, "set receive window size(num of packets)")
	flag.IntVar(&kcptun.DSCP, "dscp", 46, "set DSCP(6bit)")
	flag.IntVar(&kcptun.Conn, "conn", 1, "set num of UDP connections to server")
	...
	if !kcpOff {
		go kcpc.RunClient(cmdServer+":"+strconv.Itoa(config.ServerPort+10000), "127.0.0.1:"+strconv.Itoa(config.ServerPort+10001))
		config.Server = "127.0.0.1"
		config.ServerPort = config.ServerPort + 10001
	}
